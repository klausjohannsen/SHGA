...
void cec15_nich_func(double *, double *,int,int,int, int);

double *OShift,*M,*y,*z,*x_bound;
int n_flag,func_flag;
...
void cec15_nich_func(double *x, double *f, int nx, int mx,int func_num, int init)
{
	int cf_num=10,i,j;
	if (init)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "modules_cec15/input_data/M_%d_D%d.txt", func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "modules_cec15/input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			mexErrMsgTxt("\n Error: Cannot open input file for reading \n");
